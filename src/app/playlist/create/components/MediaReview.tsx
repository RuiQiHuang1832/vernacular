    ...
    <div className={`container mt-5 ${props.currentPage <= 2 ? "d-none" : ""}`}>
      <div className="d-flex justify-content-center">
        {props.selectedTitles.map((title,i) => (
          <div className={`${styles["breakpoint-width"]} pb-5`}  key={title.id}>
          <div  className={`d-flex mt-5 ${styles["review-container"]} justify-content-sm-between justify-content-start ${containerClassName}`}>
            <div className={`${styles["poster-container"]} col-lg-3 col text-center`}>
              ...
              <div className="d-flex justify-content-center text-primary">
                <span className={`${classNames("text-center mt-4", { [styles["multiline-ellipsis-tagline"]]: title.tagline }, { "text-danger": !title.tagline })} `}>{title.tagline || "Tagline Unavailable"}</span>
                <span className={`${classNames({ [styles["end-quote"]]: title.tagline })}`}></span>
              </div>
            </div>
            <div className={`p-5 ms-3 position-relative col-lg-9 col-7  d-md-block d-none`}>
          ...
          <div className={`${styles["review-container-accordion"]} ${containerClassName} ${styles["accordion-content"]} ${classNames({ [styles["active"]]: isReviewOpen[title.id] })}`}>
            ...
            <div className="text-center align-self-center">Write a review · {title.mediaName} ({title.mediaYear})</div>
              ...
              <div className="align-self-center">Review by User: <span className="text-success">Ben</span></div>
              </div>
