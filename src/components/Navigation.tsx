...
export default function Navigation() {
  ...
  const closeDropdown = (): void => {
   ...
   //wrong, conditionally render it in the jsx instead of doingi
  const overlay = () => {
  ...
  return ( 
    isSearchOpen ? <><div onClick={overlay} style={{zIndex:"2001", top:0, right:0}} className="float-end position-absolute" ><IoMdCloseCircle size="3em" color="yellow"/></div><Search/></> : 
    ...
    <div className={`nav-item order-lg-1 me-auto ms-3`}>
            <div className={`nav-link ${navItem}`}>
              <div style={{cursor: "pointer"}} onClick={overlay}><MdSearch /></div>
            </div>
          </div>
